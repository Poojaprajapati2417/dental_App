...
const userregister = async (req, res) => {
    ...
    try {
        ...
        const { username, password, usertype, mobileNo } = req.body;


        // check weather a field is empty or not

        if ([username, password, usertype, mobileNo].some((field) => {
            field?.trim() === ""
        })) {
            res.status(200).send(
                {
        ...
        if(!avatarLocalpath){
            return res.status(200).send(
                {
                    message:"Profile image is complusory",
                    status:"notsuccess"
                }
            )
        }
...
const loginuser = async (req, res) => {
    ...
    try {
        ...
        const { username, password, usetype } = req.body;
        ...
        if (!ispasswordvalid) {
            return res.status(200).send(
                {
                    message: "Username or password is notvalid",
                    status: "notsuccess"
